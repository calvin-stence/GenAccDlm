...
datapath = 'test_input\\Multi Thickness Surfacing Test\\structured_data\\**\\'
figure_count = 1
figure_index = 0
completed_jobs = []
with PdfPages('debug.pdf') as pdf:
    for file in glob2.glob(datapath + '*_SF_*.PMF'):
        surfaced_pmf_file = list(glob2.glob(datapath + re.sub('SF', 'FIN', os.path.basename(file))))[0]
        ddf_filepath = re.sub('0.PMF', '0.DDF', surfaced_pmf_file)
        print('Generating figures for file ' + file)
        figures_to_generate = ['RFCE', 'RFDE', 'RTCE', 'RTDE']
        pmf_list = []
        unsurfaced_pmf_obj = pmf_extractor.PmfPowermapsGet(file, figures_to_generate=figures_to_generate,
                                                           state='Before Surfacing')
        pmf_list.append(unsurfaced_pmf_obj)
        surfaced_pmf_obj = pmf_extractor.PmfPowermapsGet(surfaced_pmf_file, figures_to_generate=figures_to_generate,
                                                         state='After Surfacing')
        thickness_search = re.search(r'(\+\dmm)', surfaced_pmf_obj.file)
        if thickness_search:
            surfaced_pmf_obj.state += ' ' + thickness_search.group(1)
        pmf_list.append(surfaced_pmf_obj)
        # compare_pmf_obj = pmf_extractor.compare_pmf(unsurfaced_pmf_obj,surfaced_pmf_obj)
        # compare_pmf_obj.figures_to_generate = ['RFCM','RFDM']
        ...
        title_fig_title = surfaced_pmf_obj.lens_description + ' ' + surfaced_pmf_obj.state
        ...
        try:
            ...
            plt.suptitle(surfaced_pmf_obj.lens_description)
        ...
        for pmf_obj in pmf_list:
            figure_index = 0
            fig, axs = plt.subplots(nrows=2, ncols=len(pmf_obj.figures_to_generate),
                                    figsize=(30 / 4 * len(pmf_obj.figures_to_generate), 14))
            ...
            for fig_ids in pmf_obj.figures_to_generate:
                ...
                zi_90, x_90, y_90 = pmf_extractor.array_transect(x0_90, y0_90, x1_90, y1_90, output_data,
                                                                 smooth='cubic')
                plt.sca(axs[0][figure_index])
                plt.plot([x0 - 20, x1 - 20], [y0 - 20, y1 - 20], 'ro-', linewidth=5, markersize=20, alpha=.8)
                plt.plot([x0_90 - 20, x1_90 - 20], [y0_90 - 20, y1_90 - 20], 'bo-', linewidth=5, markersize=20,
                         alpha=.8)
